...
extra_hiddenimports += collect_submodules('dask')
extra_hiddenimports += collect_submodules('numexpr')
# extra_hiddenimports += ['rsciio.jeol']

# Common compiled libs used by scientific stack
extra_binaries = []
for _pkg in ['numpy', 'scipy', 'numexpr']:
    try:
...
a = Analysis(
    ...
    excludes=[
        ...
        'numba', 'llvmlite',  # JIT compiler (150+ MB)
        'bokeh',  # Interactive plotting (pulled by dask)
